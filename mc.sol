...
// File: contracts/BoujeToken.sol

pragma solidity 0.6.12;

contract BoujeToken is ERC20("Bouje Token", "BOUJE"), Ownable {
    uint256 constant MAX_CAP_SUPPLY = 88888 ether;
...
// MasterChef is the master of Bouje. He can make Bouje and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Bouje is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accBoujePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBoujePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accBoujePerShare; // Accumulated FISHes per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The BOUJE TOKEN!
    BoujeToken public boujeToken;
    address public devAddress;
    address public feeAddress;

    // BOUJE tokens created per block.
    uint256 public boujePerBlock = 0.05 ether;
    ...
    // The block number when BOUJE mining starts.
    uint256 public startBlock;

    uint256 constant MAX_BOUJE_SUPPLY = 88888 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 boujePerBlock);


    constructor(
        BoujeToken _boujeToken,
        uint256 _startBlock,
        address _devAddress,
        address _feeAddress
    ) public {
    ...
    // Update the given pool's BOUJE allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP
    ...
    function pendingBouje(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accBoujePerShare = pool.accBoujePerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 boujeReward = multiplier
                .mul(boujePerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accBoujePerShare = accBoujePerShare.add(
                boujeReward.mul(1e18).div(lpSupply)
            );
        }
        return user.amount.mul(accBoujePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(boujeToken.owner() != address(this)) {
        ...
        uint256 currentSupply = boujeToken.totalSupply();
        ...
        pool.accBoujePerShare = pool.accBoujePerShare.add(
            boujeReward.mul(1e18).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BOUJE allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount
    ) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accBoujePerShare).div(1e18);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accBoujePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            uint256 devReward = pending.div(10);
    ...
    // Safe boujeToken transfer function, just in case if rounding error causes pool to not have enough BOUJE.
    function safeBoujeTransfer(address _to, uint256 _amount) internal {
        uint256 boujeBal = boujeToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > boujeBal) {
            transferSuccess = boujeToken.transfer(_to, boujeBal);
        } else {
            transferSuccess = boujeToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeBoujeTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _boujeTokenPerBlock)
        external
        onlyOwner
    {
        massUpdatePools();
        boujePerBlock = _boujeTokenPerBlock;
        emit UpdateEmissionRate(msg.sender, _boujeTokenPerBlock);
    }
    ...
    function transferBoujeTokenOwnership(address _newOwner)
        external
        onlyOwner
    {
        boujeToken.transferOwnership(_newOwner);
    }
}